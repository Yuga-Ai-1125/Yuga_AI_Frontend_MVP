...
import { OurServices } from "./pages/company/OurServices";
...
function MainApp() {
  ...
  // Add new states for chapter selection
  ...
  const handleCourseClick = (course: Course) => {
    ...
    // Check if the course is one of the new NEET subjects that need chapter selection
    if (['NEET Physics', 'NEET Chemistry', 'NEET Biology'].includes(course.category)) {
      setSelectedSubject(course.category);
  ...
  const handleChapterSelect = (chapter: string, className: string) => {
    // Create a proper lesson object for the selected chapter
    const chapterLesson = {
      id: `chapter-${chapter.toLowerCase().replace(/\s+/g, '-')}`,
      title: `${chapter} - ${className}`,
      content: `Welcome to ${chapter} in ${selectedSubject} ${className}. This chapter covers fundamental concepts that are essential for your NEET preparation. Let's dive into the key topics and build a strong foundation together.`,
    ...
    // Create a proper course object for the classroom
    const chapterCourse: Course = {
      id: `course-${selectedSubject?.toLowerCase().replace(/\s+/g, '-')}-${className.toLowerCase().replace(/\s+/g, '-')}`,
      title: `${selectedSubject} - ${chapter}`,
      description: `This course covers the chapter ${chapter} of ${selectedSubject} for ${className}.`,
      image: "",
      ...
      rating: 4.5,
      students: 1000,
      tags: [selectedSubject || '', className, chapter],
      notesCount: 0
    };

    setSelectedCourse(chapterCourse);
    setSelectedLesson(chapterLesson);
    setShowChapterSelection(false);
    setSelectedSubject(null);
  };

  const handleBackFromChapterSelection = () => {
    setShowChapterSelection(false);
    setSelectedSubject(null);
  };
                ...
                <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  ...
                  {filteredCourses.length === 0 && (
                    <p className="text-blue-600 text-sm mt-1">
                      ...
                      Social Science, Computer Applications, NEET MCQ, or NEET MCQ 2.O
                    </p>
                  )}
                </div>
              )}
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1">
        {showChapterSelection && selectedSubject ? (
          <ChapterSelection
            subject={selectedSubject}
            onBack={handleBackFromChapterSelection}
