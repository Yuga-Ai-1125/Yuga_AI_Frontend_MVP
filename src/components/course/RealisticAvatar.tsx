...
export const RealisticAvatar = forwardRef<RealisticAvatarHandle, RealisticAvatarProps>(({
  ...
  const playAudioFromBase64 = (base64Audio: string) => {
    return new Promise<void>((resolve) => {
      try {
        ...
        // Play the video when audio starts - CRITICAL FOR NEET 2.O
  ...
  // CONSISTENT FEMALE VOICE SELECTION - Always use the same female voice
  const selectAndStoreFemaleVoice = (): SpeechSynthesisVoice | undefined => {
    if (typeof window === 'undefined' || !window.speechSynthesis) {
      return undefined;
    }

    // If we already selected a voice, use it consistently
    ...
    // Priority list of consistent female voices (most natural and widely available)
    const femaleVoicePriority = [
      // Windows - Natural female voices
      'Microsoft Zira Desktop',       // Female - natural (Windows default female)
      'Microsoft Hazel Desktop',      // Female - natural (UK English)
      
      // macOS - Natural female voices  
      'Samantha',                     // Female - very natural (macOS default)
      'Victoria',                     // Female - natural
      'Karen',                        // Female - natural (Australian)
      
      // Chrome/Edge - Natural female voices
      'Google UK English Female',     // Female - natural
      'Google Australian Female',     // Female - natural
      'Google US English Female',     // Female - natural
      
      // Common natural female voices across platforms
      'Moira',                        // Female - natural (Irish)
      'Tessa',                        // Female - natural (South African)
    ...
    // Remove duplicates and find the first available voice
    const uniqueVoices = [...new Set(femaleVoicePriority)];
    
    for (const voiceName of uniqueVoices) {
      const voice = voices.find(v => v.name === voiceName);
      if (voice) {
        console.log('Selected consistent female voice:', voice.name);
        selectedVoiceRef.current = voice; // Store for consistent use
        return voice;
      }
    }

    // Final fallback: return the first available female-sounding voice
    const fallbackVoice = voices.find(v => 
      v.name.toLowerCase().includes('female') || 
      v.lang.startsWith('en') // Prefer English voices
  ...
  const speakText = (text: string, onEndCallback?: () => void) => {
    ...
    if (!processedText || !soundEnabled || !isTeaching) {
  ...
  // Load voices when they become available and select consistent voice
  useEffect(() => {
    const loadVoicesAndSelect = () => {
      if (typeof window !== 'undefined' && window.speechSynthesis) {
        ...
        if (voices.length > 0) {
          ...
          // Pre-select and store the female voice for consistent use
          selectAndStoreFemaleVoice();
        }
      }
    };
