...
import { X, Mail, Lock, User, Eye, EyeOff, Loader, ArrowLeft, CheckCircle } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({ 
  ...
  const [mode, setMode] = useState<'login' | 'signup' | 'forgot-password' | 'success'>(initialMode);
  ...
  const { login, signup, loginWithProvider, resetPassword, isLoading, error } = useAuth();
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                {mode === 'signup' && (
                  <div>
                    ...
                    {validationErrors.name && (
                      <p className="text-red-600 text-xs sm:text-sm mt-1">{validationErrors.name}</p>
                    )}
                  </div>
                )}
                ...
                <div>
                  ...
                  {validationErrors.email && (
                    <p className="text-red-600 text-xs sm:text-sm mt-1">{validationErrors.email}</p>
                  )}
                </div>
                ...
                {mode !== 'forgot-password' && (
                  <div>
                    ...
                    {validationErrors.password && (
                      <p className="text-red-600 text-xs sm:text-sm mt-1">{validationErrors.password}</p>
                    )}
                  </div>
                )}
                ...
                {mode === 'signup' && (
                  <div>
                    ...
                    {validationErrors.confirmPassword && (
                      <p className="text-red-600 text-xs sm:text-sm mt-1">{validationErrors.confirmPassword}</p>
                    )}
                  </div>
                )}
                ...
                <>
                  ...
                  <div className="space-y-3">
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center px-4 py-2.5 sm:py-3 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50 text-sm sm:text-base"
                    >
                      <svg className="w-4 h-4 sm:w-5 sm:h-5 mr-3" viewBox="0 0 24 24" fill="currentColor">
